...
const StrokeText = ({
  ...
  }, [characters, numericFontSize, fontWeight, fontFamily, letterSpacing, strokeWidth]);
  ...
  useEffect(() => {
    ...
    const staggerConfig: gsap.StaggerVars | number = reverse ? { each: stagger, from: 'end' as const } : stagger;
  ...
  // Usa la altura REAL medida del texto (box.height, ya calculada con getBBox
  // sobre el tamaño de fuente que el navegador terminó renderizando —
  // funciona igual con un número fijo que con un clamp() responsive).
  // Antes de la primera medición, cae a un estimado basado en numericFontSize.
  const svgHeightPx = box ? Math.round(box.height) : Math.round(numericFontSize * 1.3);

  return (
    <span
      ref={rootRef}
      className={`stroke-text ${trigger === 'hover' ? 'stroke-text--hover' : ''} ${className}`.trim()}
      style={{ ...style, ['--stroke-text-height' as any]: `${svgHeightPx}px` }}
      role="img"
